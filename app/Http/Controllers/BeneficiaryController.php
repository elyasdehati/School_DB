...
use App\Exports\BeneficiarySummaryExport;
...
class BeneficiaryController extends Controller
{
        ...
        foreach ($classes->groupBy('province_id') as $provinceId => $provinceClasses) {
            ...
            foreach ($provinceClasses->groupBy('district_id') as $districtId => $districtClasses) {
                ...
                $maleTeachers = ProjectTeacher::where('project_id', $project->id)
                    ->whereIn('class_id', $districtClasses->pluck('id'))
                    ->where('gender', 'Male')
                    ->count();
    ...
    public function ExportBeneficiarySummary(Project $project){
        $classes = ProjectClass::with(['province', 'district'])
            ->where('project_id', $project->id)
            ->orderBy('province_id')
            ->orderBy('district_id')
        ...
        return Excel::download(
            new BeneficiarySummaryExport($reportData),
            'beneficiary_summary.xlsx'
        );
    }
}